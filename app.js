        ...
        .then(data => {
          ...
          // console.log(imageElement);
            ...
            let header =  element.querySelector('h5');
              ...
              aiResult = vehicleAI(windSpeed,humidityData,pressureData,numbers);
              console.log(aiResult)
             }
             else {
              ...
              document.querySelector('.custom').innerHTML = `<h1> ${header.textContent} <p> (Sources: theAA.com, ChatGPT) </p>  </h1> <br/>
              <ul>
  ...
  console.log(forecastUrl);
      ...
      .then(data => {
        ...
        data.list.forEach((item, index) => {
          // Process and display 5-day forecast data (similar to the previous example)
                     
                      const date = new Date(item.dt * 1000);
                      let formatDate = date.toLocaleDateString('en-US', {  month: 'short', day: 'numeric'});
                      const dayOfWeek = date.toLocaleDateString('en-US', { weekday: 'short' });
                      const newDate = `${formatDate}, ${dayOfWeek}`;
                      
  
                      // Convert headerDate to a Date object
                      let headerDate = new Date(date);
                      console.log('based on iteration',headerDate); 
                      console.log('before',index); 
                      if (date.toDateString() !== currentDate.toDateString()) {
                        // Update the current date
                        console.log(headerDate.toDateString(), 'and', currentDate.toDateString());
                        currentDate = date;
                    ...
                    if (index <= 6) {
                      ...
                      let otherDays =  daySection.querySelectorAll(`.col-md-1`);
                      console.log(otherDays);
  
                      
                  
                      if (otherDays[daysCounter]) {
          
                        otherDays[daysCounter].textContent = headerDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', weekday: 'short' });
                    } else {
                  ...
                  console.log('based on loop',headerDate);
                  daysCounter++
                 
                } 

                else {
                  currentDate.setDate(headerDate.getDate() + 3);
                  return currentDate;

                }


                let timeStamp = new Date(item.dt);
